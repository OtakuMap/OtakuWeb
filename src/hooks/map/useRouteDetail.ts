...
interface UseRouteDetailReturn {
  ...
  isLoading: boolean;
  error: Error | null;
  fetchRouteDetail: (routeId: number, placeId: number) => Promise<void>;
}
...
export const useRouteDetail = (): UseRouteDetailReturn => {
  ...
  const fetchRouteDetail = useCallback(async (routeId: number, placeId: number) => {
    try {
      setIsLoading(true);
      setError(null);

      const response = await getRouteDetail(routeId, placeId);
